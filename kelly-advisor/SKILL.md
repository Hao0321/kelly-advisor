...
```
首選：載入 C:/Users/Hao0321/.claude/skills/hao-voice/hao-voice.md
若存在 → output 格式 / tone / anti-patterns 套用 hao-voice
若 user 不是 Hao（fork 此 skill）→ 走通用風格，不套 Hao 的 voice
衝突仲裁：R-rules > hao-voice > generic default
```
...
| 設計來源 / 變現策略（私有，不公開） | `references/for-me/strategy-and-monetization.md` |
